...
<cfcomponent extends="HibachiService" accessors="true">
		...
		<cftry>
			...
			<cfif !isZipFromGithub>
				...
				<cfif (downloadedZipHash eq hashFileValue)>
					...
					<cfif fileExists( "#slatwallRootPath#/custom/assets/system/lastFullUpdate.txt.cfm" )>
						<cffile action="delete" file="#slatwallRootPath#/custom/assets/system/lastFullUpdate.txt.cfm" >
					</cfif>
	...
	<cffunction name='getMetaFolderExistsWithoutDismissalFlag'>
		<cfreturn directoryExists( expandPath('/Slatwall/meta') ) && !fileExists( expandPath('/Slatwall/custom/assets/system/metaDismiss.txt.cfm') ) />
	</cffunction>
	...
	<cffunction name='dismissMeta'>
		<cfset fileWrite( expandPath('/Slatwall/custom/assets/system') & '/metaDismiss.txt.cfm', now() ) />
	</cffunction>
