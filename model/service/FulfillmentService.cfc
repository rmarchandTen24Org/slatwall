...
component extends="HibachiService" persistent="false" accessors="true" output="false" {
		...
		for(var fulfillmentBatchItem in fulfillmentBatchItems){
			...
			if(!isNull(fulfillmentBatchItem.getOrderItem())){
				...
				if(qoh < orderItem.getQuantity()){
					stockTransferData = createStockAdjustmentDataForFulfillmentBatchItem(stockTransferData,orderItem,batchLocation);
				}
			//Handle Order Fulfillment fulfillment batch items
				...
				for(var orderFulfillmentItem in orderFulfillment.getOrderFulfillmentItems()){
					...
					if(qoh < orderFulfillmentItem.getQuantity()){
						stockTransferData = createStockAdjustmentDataForFulfillmentBatchItem(stockTransferData,orderFulfillmentItem,batchLocation);
					}
				}
			}
		}
	...
	public any function createStockAdjustmentDataForFulfillmentBatchItem(required struct stockTransferData, required any orderItem, required any batchLocation){
		...
		//Else create a new one and transfer whatever additional quantity we don't have at the batch location
			...
			stockAdjustmentItemData.quantity = arguments.orderItem.getQuantity() - QOH;
