				...
				<cfif listFindNoCase(rc.processObject.getSku().setting('skuEligibleCurrencies'), rc.order.getCurrencyCode())>
					...
					<div>
						...
						<div class="table-responsive s-gift-card-table">
							{{giftRecipientControl.orderItemGiftRecipients.length}}
							<div class="alert alert-info" role="alert" ng-show="giftRecipientControl.orderItemGiftRecipients.length == 0">Use the recipient add or search field below to add gift recipients for the gift cards selected.  You have selected <strong>5</strong> gift card/s to assign</div>
							<div ng-show="giftRecipientControl.orderItemGiftRecipients.length > 0">
						...
						<div class="form-group" ng-show="giftRecipientControl.getUnassignedCount() > 0">
							<div class="s-search-filter s-gift-card">
		                        <div class="input-group">
									...
									<span class="s-current-selection-item"></span>
	
	                				<ul ng-show="giftRecipientControl.searchText.length > 0" ng-hide="giftRecipientControl.currentGiftRecipient.firstName"><!-- display block should be replaced with js(angular) -->
	  									<!-- Item-->
