		...
		if( arrayLen(variables.Keywords) && (structCount(variables.keywordProperties) || structCount(variables.attributeKeywordProperties)) ) {
			...
			for(var ii=1; ii<=arrayLen(variables.Keywords); ii++) {
				...
				for(var attributeProperty in variables.attributeKeywordProperties) {
					...
					hqlWhere &= " EXISTS(SELECT sav.attributeValue FROM #getApplicationKey()#AttributeValue as sav WHERE sav.#fullIDMap# = #listFirst(attributeProperty, ":")# AND sav.attribute.attributeCode = '#listLast(attributeProperty,':')#' AND sav.attributeValue LIKE :#paramID# ) OR";
				}
