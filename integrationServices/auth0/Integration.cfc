...
component accessors="true" output="false" extends="Slatwall.integrationServices.BaseIntegration" implements="Slatwall.integrationServices.IntegrationInterface" {
	...
	public struct function getSettings() {
		return {
			client_name = {fieldType="text"},
			client_id = {fieldType="text"},
			userDBConnection= {fieldType="text"},
			client_secret = {fieldType="password", encryptValue="true"}
		};
	}
	
	public array function getEventHandlers() {
		return ["Slatwall.integrationServices.auth0.model.handler.SlatwallEventHandler"];
	}
	...
	public function getManagementToken(){
		if(structKeyExists(variables, 'managementToken')){
