...
component accessors="true" output="false" displayname="Stripe" implements="Slatwall.integrationServices.PaymentInterface" extends="Slatwall.integrationServices.BasePayment" {
    ...
    private void function populateRequestParamsWithMetaData(required any requestBean, required any httpRequest){
        ...
        if(len(requestBean.getOrder().getOrderID())) {
            ...
            for (var orderItem in requestBean.getOrder().getOrderItems()){
                orderItemCount++;
                httpRequest.addParam(type="formfield", name="metadata[orderItem_#orderItemCount#_id]", value="#orderItem.getOrderItemID()#");
                ...
                httpRequest.addParam(type="formfield", name="metadata[orderItem_#orderItemCount#_productName]", value="#orderItem.getSku().getProduct().getProductName()#");
                httpRequest.addParam(type="formfield", name="metadata[orderItem_#orderItemCount#_productCode]", value="#orderItem.getSku().getProduct().getProductCode()#");
            }
        }
    }
    ...
    private string function generateDescription(required any requestBean)
    {
        ...
        if(len(requestBean.getOrder().getOrderID())) {
            for (var orderItem in requestBean.getOrder().getOrderItems()){
                productNameList = listAppend(productNameList, orderItem.getSku().getProduct().getProductName());
            }
        }
