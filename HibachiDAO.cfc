...
<cfcomponent output="false" accessors="true" extends="HibachiObject">
	...
	<cfscript>
	    ...
	    public any function getSmartList(required string entityName, struct data={}){
			// Adds the Applicatoin Prefix to the entityName when needed.
			if(left(arguments.entityName, len(getApplicationKey()) ) != getApplicationKey()) {
				arguments.entityName = "#getHibachiService().getEntityMetaData('#arguments.entityName#').entityName#";
			}
		...
		public any function getAccountByAuthToken(required string authToken) {
			var account = ormExecuteQuery("SELECT acc FROM #getHibachiService().getEntityMetaData('AccountAuthentication').entityName# accauth INNER JOIN accauth.account acc WHERE (accauth.expirationDateTime is null or accauth.expirationDateTime > :now) and accauth.authToken is not null and accauth.authToken = :authToken", {now=now(), authToken=arguments.authToken}, true);
			if(isNull(account)) {
				return entityNew("#getHibachiService().getEntityMetaData('Account').entityName#");
			}
			return account;
		}
